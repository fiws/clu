...
var api = function(clu){
	...
	clu.scaleTo = function(num, cb){
		...
		workers =_.filter(workers, function(worker){
			return worker.state === "listening" || worker.state === "online";
		});
	...
	clu.stop = function(cb){
		...
		cluster.disconnect(function(){
			...
			var timeout = setTimeout(function(){
				...
				logger.warn("Could not die gracefully. Exiting now.".red);
				process.exit();
	...
	clu.status = function(cb){
		...
		_.each(workers, function(worker){
			...
			if (_.include(pendingStates, worker.status)) status.workers.pending++;
