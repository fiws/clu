...
program.on('--help', () => {
	console.log("  I have no %se what I am doing...", chalk.bold("clu"));
});
...
program
	...
	.action(() => {
		...
		if (scriptName === "commandLine" || scriptName === "commandLine.js"){
...
program
	...
	.action(() => {
		...
		socket.data("status", status => {
			console.log(chalk.white.underline("Workers\n"));
			console.log(chalk.bold("  Total:   ") + status.workers.total);
			console.log(chalk.bold.green("  Active:  ") + status.workers.active);
			console.log(chalk.bold.yellow("  Pending: ") + status.workers.pending + "\n");
			...
			console.log(chalk.white.underline("Uptime\n"));
			console.log(chalk.bold("  Master:         ") + mUptime);
			console.log(chalk.bold("  Average Worker: ") + wAvgUptime);
			console.log();
			socket.end();
		});
	});
...
socket.data("log", data => {
	if (data.msg) console.log(data.msg);
});

var tasks = {};

socket.data("progress", data => {
...
socket.data("done", data => {
	...
	if (data.err){
		console.log(chalk.red("✘ Task %s failed!"), data.name);
		console.log(data.err.message);
		if (program.verbose) console.log(data.err.stack);
	} else {
		console.log(chalk.green("\n✔ Task %s succeeded!"), data.name);
	}
...
socket.on("error", err => {
	...
	if (err.code === "ECONNREFUSED" || err.code === "ENOENT"){
		console.log(chalk.red("Master does not seem to be running."));
		console.log(chalk.red("Cannot connect to ") + socket.host);
		console.log("use --help for help");
	}
	else throw err;
});

var start = exports.start = options => {
